...
from systems.criterions import PSNR, SSIM, LPIPS, latent_manifold_loss, binary_cross_entropy, depth_smoothness_reg
...
class MarchingCubeHelper(nn.Module):
    ...
    def forward(self, level, threshold=0.):
        ...
        if self.use_torch:
            verts, faces = self.mc_func(level.to(self.rank), threshold)
...
class BaseImplicitGeometry(BaseModel):
    ...
    def isosurface_(self, vmin, vmax, group_id, group_id_dict, stage):
        x = self.helper.grid_vertices()
        ...
        def batch_func(x, group_id, group_id_dict, stage):
            rv = self.forward_level(x).cpu()
            cleanup()
            return rv
        
        # breakpoint()
        level = chunk_batch(batch_func, self.config.isosurface.chunk, True, x, group_id, group_id_dict, stage)
    ...
    @torch.no_grad()
    def isosurface(self, group_id, group_id_dict, stage):
        if self.config.isosurface is None:
            raise NotImplementedError
        mesh_coarse = self.isosurface_((-self.radius, -self.radius, -self.radius), (self.radius, self.radius, self.radius), group_id, group_id_dict, stage)
        ...
        # mesh_fine = self.isosurface_((-self.radius + 0.2, -self.radius +0.2, -self.radius+0.2), (self.radius-0.2, self.radius-0.2, self.radius-0.2), group_id, group_id_dict, stage)
        mesh_fine = self.isosurface_(vmin_, vmax_, group_id, group_id_dict, stage)
        return mesh_fine
...
class VolumeDensity(BaseImplicitGeometry):
    def setup(self):
        ...
        # breakpoint()
        # self.group_id = batch['group_id']
        self.encoding_with_network = get_encoding_with_network(self.n_input_dims, self.n_output_dims,
                                                               self.config.xyz_encoding_config,
                                                               self.config)
                                                            
    def forward(self, points, group_id=None,  group_id_dict=None, stage=None):
        points = contract_to_unisphere(points, self.radius, self.contraction_type)
        # breakpoint()
        out = self.encoding_with_network(points.view(-1, self.n_input_dims),  group_id=group_id, group_id_dict=group_id_dict, stage=stage).view(*points.shape[:-1], self.n_output_dims).float()
    ...
    #     loss_depth_smoothness = depth_smoothness_reg(self, out['depth'])
    #     loss_dict = {}
    #     loss_dict['depth_smoothness'] = loss_depth_smoothness
    #     return loss_dict
