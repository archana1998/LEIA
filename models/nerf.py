...
class NeRFModel(BaseModel):
    ...
    def isosurface(self, group_id, group_id_dict, stage):
        mesh = self.geometry.isosurface(group_id, group_id_dict, stage)
        return mesh

    def forward_(self, rays,group_id, group_id_dict, stage,**kwargs):
        n_rays = rays.shape[0]
        ...
        density, feature = self.geometry(positions, group_id, group_id_dict, stage)
        rgb = self.texture(feature, t_dirs, group_id, group_id_dict, stage)
    ...
    def forward(self, rays, group_id, group_id_dict, stage):

        if stage in ['train']:
            out = self.forward_(rays, group_id, group_id_dict, stage)
        else:
            out = chunk_batch(self.forward_, self.config.ray_chunk, True, rays, group_id, group_id_dict, stage)
            del rays
        return {
            **out,
        }
    ...
    def export(self, export_config):
        ...
        if export_config.export_vertex_color:
            _, feature = chunk_batch(self.geometry, export_config.chunk_size, True, mesh['v_pos'].to(self.rank))
